...
public class MapValueForEach extends LoopWork {
    public MapValueForEach(String klassType) {
    ...
    protected int loopItem(int start, int end, LoopMode mode, CodeMap code,
        ...
        if (!map.getValueKlass().getOriginType().equals(type))
            throw VariableException.TYPE_ERROR.getThrow(map.getValueKlass().getOriginType() + " != " + type);
