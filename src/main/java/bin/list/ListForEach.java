...
public class ListForEach extends LoopWork {
    public ListForEach(String klassType) {
    ...
    protected int loopItem(int start, int end, LoopMode mode, CodeMap code,
        ...
        CustomList<?> list = (CustomList<?>) klassValue;
        // ex) } <= ㅇㅁㅇ 변수명
        String[] tokens = mode.getToken(code.get(end)).split("\\s", 2);
        String type = tokens[0].strip();    // ㅇㅁㅇ
        String name = tokens[1].strip();    // 변수명
        if (!list.getTypes().getOriginType().equals(type))
            throw VariableException.TYPE_ERROR.getThrow(list.getTypes().getOriginType() + " != " + type);
