...
public class Sonic {
    public void start(AudioItem item) {
        try {
            int loop = item.getLoop();
            for (int i = 0; i < loop; i++) {
                AudioInputStream stream = item.getStream();
                AudioFormat format = stream.getFormat();
                int sampleRate = (int) format.getSampleRate();
                int numChannels = format.getChannels();

                int fm = (int)stream.getFrameLength() * format.getFrameSize();
                SourceDataLine.Info info = new DataLine.Info(SourceDataLine.class, format, fm);
                SourceDataLine line = (SourceDataLine) AudioSystem.getLine(info);
                line.open(stream.getFormat());
                line.start();
                // 초기값
                this.sonicReset(sampleRate, numChannels);
                this.setSpeed(item.getSpeed());
                this.setPitch(item.getPitch());
                this.setRate(item.getRate());
                this.setVolume(item.getVolume());
                this.setUseChordPitch(item.isEmulateChordPitch());
                this.setQuality(item.getQuality());
                this.play(stream, line);
                line.drain();
                line.stop();
            }
        } catch (LineUnavailableException e) {
            throw FileException.DO_NOT_READ.getThrow(item);
        } catch (IOException e) {
            throw SystemException.CREATE_ERROR.getThrow(item);
        }
    }

    private void play(AudioInputStream audioStream, SourceDataLine line) throws IOException {
        ...
        do {
            ...
            if(numRead <= 0) flushStream();
            else writeBytesToStream(inBuffer, numRead);
            do {
                numWritten = readBytesFromStream(outBuffer, bufferSize);
                if(numWritten > 0) line.write(outBuffer, 0, numWritten);
            } while(numWritten > 0);
        } while(numRead > 0);
    }
    ...
    private static final int SONIC_FILTER_POINTS = 12;
    private static final int SONIC_TABLE_SIZE = 601;
    private static final short[] sonicTable = {
    ...
    @Getter @Setter private boolean useChordPitch;
    @Getter @Setter private int quality;
    @Getter private float rate;
    @Getter private int numChannels;
    @Getter private int numOutputSamples;
    ...
    public void sonicReset(int sampleRate, int numChannels) {
    ...
    public int readBytesFromStream(byte[] outBuffer, int maxBytes) {
        ...
        if(numSamples == 0 || maxSamples == 0) return 0;
        if(numSamples > maxSamples) {
            remainingSamples = numSamples - maxSamples;
            numSamples = maxSamples;
        }
    ...
    private boolean prevPeriodBetter(int minDiff, int maxDiff, boolean preferNewPeriod) {
        ...
        } else return minDiff > prevMinDiff;
    }
    ...
    private int findPitchPeriod(short[] samples, int position, boolean preferNewPeriod) {
        ...
        if(sampleRate > SONIC_AMDF_FREQ && quality == 0) skip = sampleRate/SONIC_AMDF_FREQ;
        if(numChannels == 1 && skip == 1) {
            ...
            if(skip != 1) {
                ...
                if(numChannels == 1) period = findPitchPeriodInRange(samples, position, minP, maxP);
                else {
    ...
    private void overlapAdd(int numSamples, int numChannels,
                            short[] out, int outPos,
                            short[] rampDown, int rampDownPos,
                            short[] rampUp, int rampUpPos) {
    ...
    // Overlap two sound segments, ramp the volume of one down, while ramping the
    // other one from zero up, and add them, storing the result at the output.
    private void overlapAddWithSeparation(int numSamples, int numChannels, int separation,
                                          short[] out, int outPos,
                                          short[] rampDown, int rampDownPos,
                                          short[] rampUp, int rampUpPos) {
        for(int i = 0; i < numChannels; i++) {
    ...
    private int findSincCoefficient(int i, int ratio, int width) {
        int lobePoints = (SONIC_TABLE_SIZE-1)/SONIC_FILTER_POINTS;
        int left = i*lobePoints + (ratio*lobePoints)/width;
        int right = left + 1;
        int position = i*lobePoints*width + ratio*lobePoints - left*width;
        int leftVal = sonicTable[left];
        int rightVal = sonicTable[right];
    ...
    private short interpolate(short[] in, int inPos, int oldSampleRate, int newSampleRate) {
        ...
        for (i = 0; i < SONIC_FILTER_POINTS; i++) {
    ...
    private void adjustRate(float rate, int originalNumOutputSamples) {
        ...
        moveNewSamplesToPitchBuffer(originalNumOutputSamples);
        // Leave at least N(SINC_FILTER_POINTS) pitch samples in the buffer
        for(position = 0; position < numPitchSamples - SONIC_FILTER_POINTS; position++) {
    ...
    public int changeFloatSpeed(float[] samples, int numSamples,
                                float speed, float pitch, float rate, float volume,
                                boolean useChordPitch, int sampleRate, int numChannels) {
        sonicReset(sampleRate, numChannels);
        setSpeed(speed);
        setPitch(pitch);
        setRate(rate);
        setVolume(volume);
        setUseChordPitch(useChordPitch);
        writeFloatToStream(samples, numSamples);
        flushStream();
        numSamples = numOutputSamples;
        readFloatFromStream(samples, numSamples);
        return numSamples;
    }

    /* This is a non-stream oriented interface to just change the speed of a sound sample */
    public int sonicChangeShortSpeed(short[] samples, int numSamples,
                                     float speed, float pitch, float rate, float volume,
                                     boolean useChordPitch, int sampleRate, int numChannels) {
        sonicReset(sampleRate, numChannels);
        setSpeed(speed);
        ...
        setVolume(volume);
        setUseChordPitch(useChordPitch);
        writeShortToStream(samples, numSamples);
        flushStream();
        numSamples = numOutputSamples;
        readShortFromStream(samples, numSamples);
        return numSamples;
    }
}
